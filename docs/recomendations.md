based on thee documentation from fastapi
...
Can we do a more efficient work at the time of detect multpiple results since we are using a concurrent fast api server more scalable for make the work that you mention more scalable?


is that possible?

I want to add a dedicared fast api server, ( or the oficial documenation fron deepface already acomplish this in some way?)  for handle concurrent calls from my clients that are going to requiere to do this calculations and face administrations,

What do you think is the better option? maybe we can also administrate our sqlite db for handle additional information for cruds and so on for black lists and other use case, what you think abouht this idea?